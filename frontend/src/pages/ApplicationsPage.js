...
const ApplicationsPage = () => {
  ...
  return (
    <div className="space-y-8 dark:text-gray-100">
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-blue-500 via-indigo-500 to-indigo-700 rounded-3xl p-6 shadow-lg">
        ...
        <h1 className="text-2xl md:text-3xl font-black text-white mb-1 relative z-10">📋 Application Planner</h1>
        <p className="text-blue-100/90 text-sm relative z-10">Track tasks, manage documents, and review your SOP & LOR with AI</p>
      </div>
                    ...
                    <tr key={docType} className="hover:bg-gray-50/50 dark:hover:bg-gray-900/10">
                      <td className="py-4 px-4 font-semibold text-gray-800 dark:text-white">
                        <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                          ...
                          {docType === 'Letter of Recommendation 1 (LOR 1)' && (
                            <button
                              onClick={() => setShowLorReviewer(true)}
                              className="inline-flex items-center gap-1 bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-1 px-2.5 rounded-xl text-[10px] transition-all hover:scale-105 active:scale-95 shadow-sm shrink-0"
                            >
                              ✨ {lang === 'en' ? 'AI Review' : 'এআই রিভিউ'}
                            </button>
                          )}
                        </div>
                      </td>
      ...
      {showLorReviewer && (
        <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fadeIn">
          <div className="bg-white dark:bg-gray-800 w-full max-w-4xl rounded-3xl shadow-2xl border border-gray-150 dark:border-gray-700 overflow-hidden transform transition-all duration-300 max-h-[90vh] flex flex-col">
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-6">
              {/* Target Information Badge */}
              <div className="bg-indigo-50 dark:bg-indigo-950/20 border border-indigo-100 dark:border-indigo-900/50 rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  <p className="text-xs font-bold text-indigo-800 dark:text-indigo-400 uppercase tracking-wider">
                    {lang === 'en' ? 'Target Application Profiles' : 'আবেদনের লক্ষ্য বিবরণ'}
                  </p>
                  <div className="text-sm font-semibold text-gray-700 dark:text-gray-300 mt-1 flex flex-wrap gap-x-4 gap-y-1">
                    <span>📚 {lang === 'en' ? 'Field' : 'বিভাগ'}: {profile?.field_of_interest || (lang === 'en' ? 'Not Set (Set in profile)' : 'সেট করা নেই')}</span>
              ...
              <form onSubmit={handleReviewLor} className="space-y-4">
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full h-48 border border-gray-200 dark:border-gray-700 rounded-2xl p-4 text-sm bg-gray-50 dark:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 text-gray-800 dark:text-gray-100"
                    required
                ...
                <div className="flex justify-between items-center">
                  <p className="text-xs text-gray-400">
                    {lang === 'en' ? `${lorText.split(/\s+/).filter(Boolean).length} words` : `${lorText.length} টি অক্ষর`}
                  </p>
                  <div className="flex gap-2">
                    <button
                      type="button"
                    ...
                    <button
                      ...
                      className="px-6 py-2 bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-700 hover:to-blue-700 text-white rounded-xl text-sm font-bold shadow-md hover:shadow-lg disabled:opacity-50 transition-all flex items-center gap-2"
                    >
                      {loadingLorReview ? (
                        <>
                          <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white animate-scan"></div>
                        ...
                        <>
                          <span>✨</span>
                          <span>{lang === 'en' ? 'Review LOR' : 'এলওআর রিভিউ করুন'}</span>
                        </>
                      )}
                    </button>
                  </div>
              ...
              {loadingLorReview && (
                <div className="bg-gray-50 dark:bg-gray-900/40 border border-gray-100 dark:border-gray-700 rounded-2xl p-8 flex flex-col items-center justify-center space-y-4 animate-pulse relative overflow-hidden min-h-[250px]">
                  ...
                  <div className="w-16 h-16 rounded-full border-4 border-t-indigo-500 border-r-blue-500 border-b-purple-500 border-l-transparent animate-spin"></div>
                  
                  <div className="text-center space-y-2">
                    <p className="font-extrabold text-indigo-700 dark:text-indigo-400 text-sm animate-bounce">
                      {lang === 'en' ? 'AI Review Processing...' : 'এআই রিভিউ প্রসেস করা হচ্ছে...'}
                    </p>
                    <p className="text-xs text-gray-500 dark:text-gray-400 font-medium">
              ...
              {lorReviewResult && !loadingLorReview && (
                <div className="bg-gray-50 dark:bg-gray-900/50 border border-gray-150 dark:border-gray-700 rounded-2xl p-6 space-y-4 animate-fadeIn">
                  <div className="flex justify-between items-center border-b border-gray-200 dark:border-gray-700 pb-3">
                    <h4 className="font-black text-gray-800 dark:text-white flex items-center gap-2">
                      ✉️ {lang === 'en' ? 'AI Evaluation Report' : 'এআই মূল্যায়ন রিপোর্ট'}
                    </h4>
                    <span className="text-xs bg-green-100 text-green-800 dark:bg-green-950/30 dark:text-green-400 font-extrabold px-2.5 py-1 rounded-full">
                      {lang === 'en' ? 'Ready' : 'প্রস্তুত'}
                    </span>
                  ...
                  <div className="prose dark:prose-invert max-w-none">
                    <ReactMarkdown
                      remarkPlugins={[remarkGfm]}
                      components={{
                        h3: ({node, ...props}) => <h3 className="text-base font-black text-indigo-700 dark:text-indigo-400 mt-4 mb-2 flex items-center gap-1.5" {...props} />,
                        p: ({node, ...props}) => <p className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed mb-3 font-medium" {...props} />,
                        ul: ({node, ...props}) => <ul className="list-disc pl-5 mb-3 text-sm text-gray-600 dark:text-gray-300 space-y-1.5" {...props} />,
                        li: ({node, ...props}) => <li className="pl-1" {...props} />,
                        strong: ({node, ...props}) => <strong className="font-black text-gray-900 dark:text-white" {...props} />,
